...
## Motivación: el problema de la regresión simple

`\lettrine[lines=3]{E}{l}`{=latex} modelo de regresión lineal simple
$$
y = \beta_0 + \beta_1 x + u
$$
...
El estimador de mínimos cuadrados ordinarios (MCO) elige $\hat{\beta}$ minimizando la suma de cuadrados de los residuos
$$
\mathrm{SCR}(\beta) \;=\; (\mathbf{y}-\mathbf{X}\beta)'(\mathbf{y}-\mathbf{X}\beta)
\;=\; \mathbf{y}'\mathbf{y} - 2\beta'\mathbf{X}'\mathbf{y} + \beta'\mathbf{X}'\mathbf{X}\beta.
$$

Derivando respecto de $\beta$ e igualando a cero obtenemos las **ecuaciones normales**
$$
...
Los valores ajustados son $\hat{\mathbf{y}} = \mathbf{X}\hat{\beta}$ y los residuos $\hat{\mathbf{u}} = \mathbf{y} - \hat{\mathbf{y}}$. Por construcción, las ecuaciones normales implican $\mathbf{X}'\hat{\mathbf{u}} = \mathbf{0}$: los residuos son ortogonales a todos los regresores, incluida la constante. En particular, la media muestral de $\hat{\mathbf{u}}$ es cero siempre que el modelo contenga un intercepto.
...
## Supuestos de Gauss--Markov para RLM
...
## Insesgadez de MCO (forma matricial)
...
$$
Por la ley de esperanzas iteradas, $\mathbb{E}[\hat{\beta}] = \beta$ también de forma incondicional. $\quad\square$
...
## Varianza de los estimadores MCO (forma matricial)
...
## Práctica: Regresión Múltiple en R
