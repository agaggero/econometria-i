...
> *Estado: portado 2026-05-18. Revisión editorial: pendiente. Contenido enriquecido respecto al inglés (G9 — fuente española es canónica).*
...
En forma matricial, $\mathbf{Y} = \mathbf{X}\boldsymbol{\beta} + \mathbf{u}$, exactamente como antes —solo cambia el índice—. Un ejemplo típico es una serie mensual de datos de una empresa:
...
Aplicando MCO a $(\mathbf{Y}^*, \mathbf{X}^*)$ se obtiene $\hat{\boldsymbol\beta}_{\text{MCG}}$.

### 8.4.2 El procedimiento iterativo de Cochrane-Orcutt

La pega: $\rho$ es desconocido. Hay que estimarlo. El procedimiento de **Cochrane-Orcutt (CO)** es el algoritmo de MCG factibles más sencillo: alternar entre estimar $\rho$ a partir de los residuos y reestimar $\boldsymbol\beta$ sobre los datos cuasi-diferenciados, hasta que $\rho$ se estabilice.
...
4. Aplicar MCO sobre $(y_t^*, x_t^*)$ para obtener nuevos coeficientes $\hat{\boldsymbol\beta}^{(2)}$ y nuevos residuos $\hat u_t^{(2)}$.
